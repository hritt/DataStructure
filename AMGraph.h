...
void AMGraph<T>::Prim(int v)
{
	...
	for(i=1;i<vexnum;i++){ //time:vexnum-1
...
void AMGraph<T>::Kruskal()
{
	...
	k=0;
	for(i=0;i<vexnum;i++){
		for(j=i;j<vexnum;j++){ //upper triangle
			if(edges[i][j]!=INFINITY){
				edge2[k].head=i;
				edge2[k].tail=j;
	...
	for(i=0;i<vexnum;i++)
		components[i]=i;
	j=0;
	for(i=1;i<vexnum;){ //time:vexnum-1
		int h=edge2[j].head;
		int t=edge2[j].tail;
...
void AMGraph<T>::Dijkstra()
{
	...
	for(i=1;i<vexnum;i++){//time:vexnum-1
		//update distences
		for(j=0;j<vexnum;j++){
			if(set[j]==false && edges[v][j]!=INFINITY && dist[v]+edges[v][j]<=dist[j]){
